      ...
      <section ref={contentRef} className="relative z-10 h-[100dvh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 min-h-0 px-4 md:px-8 pb-4 md:pb-5 flex items-center justify-center">
          <div className="w-full max-w-[1400px] grid grid-cols-1 lg:grid-cols-[1.8fr_0.55fr] gap-4 lg:gap-6 items-center" style={{ minHeight: '80vh' }}>
            ...
            {/* Left side — glass panel (unchanged) */}
            ...
            <div className="animate-fade-in-up animation-delay-200 flex flex-col items-center justify-center gap-6 py-12 lg:py-0">
              <h2
                className="text-4xl md:text-5xl font-bold tracking-tight text-center drop-shadow-md"
              ...
              <p
                ...
                style={{ color: 'hsl(150 8% 30%)' }}
              >
                Begin your journey of self-expression through play.
              </p>
              <button
                onClick={() => navigate("/login")}
                className="group relative w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 shadow-lg hover:shadow-xl"
                style={{
                  background: 'linear-gradient(135deg, hsl(var(--primary)), hsl(var(--primary) / 0.8))',
                }}
                aria-label="Get Started"
              >
                <ArrowRight className="w-7 h-7 text-primary-foreground transition-transform duration-300 group-hover:translate-x-1" />
                {/* Pulse ring */}
                <span className="absolute inset-0 rounded-full animate-ping opacity-20" style={{ background: 'hsl(var(--primary) / 0.4)' }} />
              </button>
            </div>
          </div>
        </div>
